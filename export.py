...
def onnx_Metadata_sim(f, model, simplify=True):
    # Metadata
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='./runs/train/exp6/weights/best_2.pt', help='weights path')  # from yolov5/models/
    ...
    # ====================== 🔥 自动兼容 3层 / 4层 Detect 🔥 ======================
    detect_layer = model.model[-1]  # 拿到Detect层
    nl = detect_layer.nl  # 自动获取：模型真实有几层检测头

    # 删除旧 anchor_grid
    if hasattr(detect_layer, 'anchor_grid'):
        delattr(detect_layer, 'anchor_grid')

    # ✅ 自动创建对应层数的 anchor_grid（3层/4层自动适配）
    detect_layer.anchor_grid = [torch.zeros(1)] * nl

    # ✅ 自动设置 stride（兼容3层/4层）
    if nl == 3:
        model.stride = torch.tensor([8, 16, 32], dtype=torch.float32)
    elif nl == 4:
        model.stride = torch.tensor([8, 16, 32, 64], dtype=torch.float32)
    detect_layer.stride = model.stride
    # ==========================================================================
    ...
    # Metadata
    onnx_Metadata_sim(f, model, simplify=True)

...
def build_targets(p, targets, model):
    ...
    for i in range(det.nl):
        ...
        # 修改为（强制转换+避免原地操作）
        gj_int = torch.clamp(gj.round().long(), 0, int(gain[3] - 1))
        gi_int = torch.clamp(gi.round().long(), 0, int(gain[2] - 1))
        indices.append((b, a, gj_int, gi_int))  # image, anchor, grid indices
